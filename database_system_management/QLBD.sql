...
SELECT CT.* FROM CAUTHU CT JOIN CAULACBO CLB ON CT.MACLB = CLB.MACLB WHERE CT.MAQG = 'VN' AND CLB.TENCLB = N'BECAMEX BÌNH DƯƠNG'

-- 5. Cho biết mã số, họ tên, ngày sinh, địa chỉ và vị trí của các cầu thủ thuộc đội bóng "SHB Đà Nẵng" có quốc tịch "Braxin".
SELECT MACT, HOTEN, NGAYSINH, CT.DIACHI, VITRI FROM CAUTHU CT JOIN CAULACBO CLB ON CT.MACLB = CLB.MACLB JOIN QUOCGIA QG ON CT.MAQG = QG.MAQG WHERE CLB.TENCLB = N'SHB ĐÀ NẴNG' AND Q.TENQG = N'Bra-xin';

-- 6. Hiển thị thông tin các cầu thủ đang thi đấu trong câu lạc bộ sân nhà "Long An".
SELECT CT.* FROM CAUTHU CT JOIN CAULACBO CLB ON CT.MACLB = CLB.MACLB JOIN SANVD SVD ON CLB.MASAN = SVD.MASAN WHERE SVD.TENSAN = N'Long An';

-- 7. Cho biết kết quà (MATRAN, NGAYTD, TENSAN, TENCLB1, TENCLB2, KETQUA) các trận đấu vòng 2 của mùa bóng năm 2025.
SELECT TD.MATRAN, TD.NGAYTD, SVD.TENSAN, C1.TENCLB, C2.TENCLB, TD.KETQUA FROM TRANDAU TD JOIN SANVD SVD ON TD.MASAN = SVD.MASAN JOIN CAULACBO C1 ON TD.MACLB1 = C1.MACLB JOIN CAULACBO C2 ON TD.MACLB2 = C2.MACLB WHERE TD.VONG = 2 AND TD.NAM = 2025;
...
-- 13. Cho biết mã câu lạc bộ, tên câu lạc bộ, tên sân vận động, địa chỉ và số lượng cầu thủ nuớc ngoài (có quốc tịch khác "Viet Nam") tương ứng của các câu lac bộ có nhiều hơn 2 cầu thủ nước ngoài.
SELECT CLB.MACLB, CLB.TENCLB, SVD.TENSAN, SVD.DIACHI, COUNT(CT.MACT) FROM CAULACBO CLB JOIN SANVD SVD ON CLB.MASAN = SVD.MASAN JOIN CAUTHU CT ON CLB.MACLB = CT.MACLB WHERE CT.MAQG <> 'VN' GROUP BY CLB.MACLB, CLB.TENCLB, SVD.TENSAN, SVD.DIACHI HAVING COUNT(CT.MACT) > 2;

-- 14.Cho biết tên tỉnh, số lượng cầu thủ đang thi đấu ở vị trí tiền đạo trong các câu lạc bộ thuốc địa bàn tỉnh đó quản lý.
SELECT T.TENTINH, COUNT(CT.MACT) FROM TINH T JOIN CAULACBO CLB ON T.MATINH = CLB.MATINH JOIN CAUTHU CT ON CLB.MACLB = CT.MACLB WHERE CT.VITRI = N'Tiền đạo' GROUP BY T.TENTINH;

-- 15. Cho biết tên câu lạc bộ, tên tỉnh mà CLB đang đóng nằm ở vị trí cao nhất của bảng xếp hạng của vòng 3, năm 2025.
SELECT CLB.TENCLB, T.TENTINH FROM BANGXH BXH JOIN CAULACBO CLB ON BXH.MACLB = CLB.MACLB JOIN TINH T ON CLB.MATINH = T.MATINH WHERE BXH.VONG = 3 AND BXH.NAM = 2025 AND BXH.HANG = 1;

-- 16.Cho biết tên huấn luyện viên đang năm giữ một vị trí trong một câu lạc bộ mà chưa có số điện thoại.
SELECT HLV.TENHLV FROM HUANLUYENVIEN HLV JOIN HLV_CLB HC ON HLV.MAHLV = HC.MAHLV WHERE HLV.DIENTHOAI IS NULL OR HLV.DIENTHOAI = '';
...
SELECT CT.HOTEN FROM CAUTHU CT JOIN BANGXH BXH ON CT.MACLB = BXH.MACLB WHERE BXH.VONG = 3 AND BXH.NAM = 2025 AND (BXH.HANG > 6 OR BXH.HANG < 3);
...
SELECT TD.NGAYTD, SVD.TENSAN, C1.TENCLB, C2.TENCLB, TD.KETQUA FROM TRANDAU TD JOIN SANVD SVD ON TD.MASAN = SVD.MASAN JOIN CAULACBO C1 ON TD.MACLB1 = C1.MACLB JOIN CAULACBO C2 ON TD.MACLB2 = C2.MACLB WHERE (TD.MACLB1 = (SELECT MACLB FROM BANGXH WHERE VONG = 3 AND NAM = 2025 AND HANG = 1) OR TD.MACLB2 = (SELECT MACLB FROM BANGXH WHERE VONG = 3 AND NAM = 2025 AND HANG = 1)) AND TD.VONG <= 3 AND TD.NAM = 2025;

-- 20.Cho biết NGAYTD, TENCLB1, TENCLB2, KETQUA các trân đấu diễn ra vào tháng 3 trên sân nhà mà không bị thủng lưới.
SELECT TD.NGAYTD, C1.TENCLB, C2.TENCLB, TD.KETQUA FROM TRANDAU TD JOIN CAULACBO C1 ON TD.MACLB1 = C1.MACLB JOIN CAULACBO C2 ON TD.MACLB2 = C2.MACLB WHERE MONTH(TD.NGAYTD) = 3 AND RIGHT(TD.KETQUA, 1) = '0';
...
SELECT CLB.TENCLB FROM CAULACBO CLB JOIN HLV_CLB HC ON CLB.MACLB = HC.MACLB JOIN HUANLUYENVIEN HLV ON HC.MAHLV = HLV.MAHLV WHERE HC.VAITRO = N'HLV Chính' AND DAY(HLV.NGAYSINH) = 20 AND MONTH(HLV.NGAYSINH) = 5;
...
SELECT TOP 1 CLB.TENCLB, T.TENTINH FROM BANGXH BXH JOIN CAULACBO CLB ON BXH.MACLB = CLB.MACLB JOIN TINH T ON CLB.MATINH = T.MATINH WHERE BXH.NAM = 2025 ORDER BY BXH.HANG ASC;
...
SELECT TD.* FROM TRANDAU TD WHERE TD.MACLB1 = (SELECT TOP 1 MACLB FROM BANGXH WHERE VONG = 3 AND NAM = 2025 ORDER BY HANG DESC) OR TD.MACLB2 = (SELECT TOP 1 MACLB FROM BANGXH WHERE VONG = 3 AND NAM = 2025 ORDER BY HANG DESC);
...
WITH XepHang AS (
    SELECT
        CT.MACT,
        CT.HOTEN,
        SUM(TGT.SOBANTHANG) AS SoBan,
        CLB.TENCLB,
        ROW_NUMBER() OVER (
            PARTITION BY CLB.MACLB
            ORDER BY SUM(TGT.SOBANTHANG) DESC, CT.MACT
        ) AS Hang
    FROM CAUTHU CT
    JOIN CAULACBO CLB ON CT.MACLB = CLB.MACLB
    JOIN THAMGIATRAN TGT ON CT.MACT = TGT.MACT
    GROUP BY CT.MACT, CT.HOTEN, CLB.MACLB, CLB.TENCLB
)
SELECT MACT, HOTEN, SoBan, TENCLB
FROM XepHang
WHERE Hang <= 3 AND SoBan > 0;
GO
...
SELECT
    CT.MACT,
    CT.HOTEN,
    COUNT(DISTINCT TGT.VITRI_TD) AS SoViTriDaThiDau
FROM CAUTHU CT
JOIN THAMGIATRAN TGT ON CT.MACT = TGT.MACT
GROUP BY CT.MACT, CT.HOTEN
HAVING COUNT(DISTINCT TGT.VITRI_TD) >= 2;
GO
...
SELECT TOP 1 WITH TIES
    CLB.MACLB,
    CLB.TENCLB,
    SUM(TGT.SOTHEVANG + TGT.SOTHEDO) AS TongSoThe
FROM THAMGIATRAN TGT
JOIN TRANDAU TD ON TGT.MATRAN = TD.MATRAN
JOIN CAUTHU CT ON TGT.MACT = CT.MACT
JOIN CAULACBO CLB ON CT.MACLB = CLB.MACLB
WHERE TD.NAM = 2025
GROUP BY CLB.MACLB, CLB.TENCLB
ORDER BY TongSoThe ASC;